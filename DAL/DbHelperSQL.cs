...
namespace DAL
{
    public class DbHelperSQL
    {
        ...
        /// <returns></returns>
        public static DataTable GetPageListByProc(string proName, int page_no, int page_size, out int total_row_number, out int total_page_number)
        {
            ...
            using (SqlConnection conn = new SqlConnection(strConn))
            {
                ...
                //设置 参数
                da.SelectCommand.Parameters.AddWithValue("@PAGE_NO", page_no);//当前页码,要修改@ 的名字，改成你自己的存储过程的参数名称
                da.SelectCommand.Parameters.AddWithValue("@PAGE_SIZE", page_size);//每页的记录数
                //da.SelectCommand.Parameters.AddWithValue("@IS_DEL", isDel);

                //在存储过程中 输出参数
                da.SelectCommand.Parameters.Add(new SqlParameter("@TOTAL_ROW_NUMBER", SqlDbType.Int));
                da.SelectCommand.Parameters.Add(new SqlParameter("@TOTAL_PAGE_NUMBER", SqlDbType.Int));
                ...
                da.Fill(dt);
                //获得 存储过程 返回的 输出参数
                total_row_number = Convert.ToInt32(da.SelectCommand.Parameters[2].Value);
                total_page_number = Convert.ToInt32(da.SelectCommand.Parameters[3].Value);
            }
            //返回数据表
            return dt;
        }
